...
SkyLevel.prototype = {
	...
	//sprites proload
		...
		game.load.image('PART_rain', '/img/rain_particle.png');
	//atlas preload
    ...
    create: function() {
        ...
        //player
        player = new PlayerTEMP(game, 'pip_Dandelion', 1, 0); //PlayerTEMP prefab with scale 1 and rotation 0 -- uses WASD movement
    ...
    update: function() {
        ...
        if (game.physics.arcade.collide(player, enemy)){
            ...
            player.animations.frame = player.animations.frame + 1;
            //player.kill();
        } else {
    ...
    createFuel: function(totalFuel){
        ...
        for (var i = 0; i<totalFuel; i++){
            xLoc = game.rnd.integerInRange(100, game.world.width);
            yLoc = game.rnd.integerInRange(100, game.world.height);
            fuel[i] = new Obstacle(game, 'mentos', 1, xLoc, yLoc);
